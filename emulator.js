import { instructions } from "./rom/instructions.js";
import { base16Instructions } from "./rom/base16Instructions.js";
...
class Emulator {
  constructor() {
    ...
    this.base16Instructions = base16Instructions;
    this.registers = {
      // 8-bit registers
      A: 0,
      B: 0,
    ...
    this.stack = new Array(65536).fill(0);
  ...
  // Calculate the parity of a byte (even parity returns 1, odd parity returns 0)
  calculateParity(val) {
    let bit_count = 0;
    for (let i = 0; i < 8; i++) {
  ...
  // Load the instructions from the disassembler
  loadInstructions(data) {
    // use this array to get base 16 bytes
    this.instructions = data.map((instruction) => {
      return parseInt(instruction, 16);
    });

    // use this array to get the opcode
    this.base16Instructions = data;
    console.log("base16Instructions", this.base16Instructions);
  }

  drawToCanvas(memory) {
    const VIDEO_MEMORY_START = 0x2400;
    const VIDEO_MEMORY_END = 0x3fff;  // 0x2400 - 0x3fff is the video memory range
    ...
    for (let y = 0; y < SCREEN_HEIGHT; y++) {
      for (let xByte = 0; xByte < SCREEN_WIDTH / 8; xByte++) {
        const byte = memory[VIDEO_MEMORY_START + y * (SCREEN_WIDTH / 8) + xByte] || 0;
  ...
  drawUI() {
    const loop = () => {
      ...
      const currentInstruction = this.base16Instructions[this.registers.PC];
      // UI updates
      document.getElementById("current-instruction").innerText =
        `Current Instruction: ${currentInstruction} (hex PC: ${this.registers.PC.toString(16)})`;

      document.getElementById("program-counter").innerText =
        `Program Counter: ${this.registers.PC}`;
  ...
  gameLoop() {
    ...
    const cyclesToExecute = cyclesPerMillisecond / 1000; // Cycles per millisecond

    while (this.registers.PC < this.instructions.length && !this.missing) {
      ...
      while (cyclesExecuted < cyclesToExecute && this.registers.PC < this.instructions.length && !this.missing) {
        this.executeInstruction(this.base16Instructions[this.registers.PC]);
        cyclesExecuted += this.getInstructionCycles(this.base16Instructions[this.registers.PC]);
        i++;
      }
    }
    this.drawToCanvas(this.memory);
  }
  ...
  getInstructionCycles(instruction) {
    switch (instruction) {
      case "cf": return 4; 
      case "d7": return 4;
      default: return 4; // Default cycle count (if unknown)
    }
  }
  ...
  executeInstruction(instruction) {
    ...
    switch (instruction) {
      ...
      case "32":
        address = (this.instructions[this.registers.PC + 2] << 8) | this.instructions[this.registers.PC + 1];
        this.memory[address] = this.registers.A;
        console.log("Added A to memory at address:", address.toString(16), "value:", this.registers.A.toString(16));
        this.registers.PC += 3;
        break;
      ...
      case "35":
        ...
        this.registers.PC++;
        // Set flags
        this.registers.flags.Z = this.memory[address] === 0 ? 1 : 0;
        this.registers.flags.S = (this.memory[address] & 0x80) ? 1 : 0;
        this.registers.flags.AC = ((this.memory[address] & 0x0f) === 0x0f) ? 1 : 0;
        this.registers.flags.P = this.calculateParity(this.memory[address]);
        break;
      ...
      case "cf":
        ...
        this.registers.PC = 0x08; // Jump to interrupt vector for RST 1
        break;
      ...
      case "d7":
        ...
        this.registers.PC = 0x10; // Jump to the RST 2 interrupt vector
        break;
      ...
      case "db":
        ...
        console.log("Input from port:", portNumber.toString(16));
      ...
      case "d3":
        ...
        console.log("Output to port:", port.toString(16));
...
// emulator.loadInstructions(disassembler.instructions);
// emulator.drawUI();
emulator.gameLoop();
